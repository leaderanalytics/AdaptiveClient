...
public class CallTests : BaseTest
{
    ...
    public void Reslove_InProcessClient3_of_type_IDummyAPI1()
    {
        ...
        Moq.Mock<IEndPointValidator> MySQL_Validator_Mock = new();
        ...
        IAdaptiveClient<IDummyAPI1> client1 = container.Resolve<IAdaptiveClient<IDummyAPI1>>();
        string result = client1.Call(x => x.GetString());
        Assert.That("Application_MySQL1", Is.EqualTo(client1.CurrentEndPoint.Name));
        Assert.That("InProcessClient3", Is.EqualTo(result) );
    }
    ...
    {
        Moq.Mock<IEndPointValidator> InProcess_Validator_Mock = new();
        ...
        Assert.That("Application_MySQL1", Is.EqualTo(client1.CurrentEndPoint.Name));
        Assert.That("InProcessClient3", Is.EqualTo(result2));
    }
    ...
    public void Reslove_WebAPIClient_of_type_IDummyAPI1()
    {
        ...
        IAdaptiveClient<IDummyAPI1> client1 = container.Resolve<IAdaptiveClient<IDummyAPI1>>();
        string result = client1.Call(x => x.GetString());
        Assert.That("Application_WebAPI1", Is.EqualTo(client1.CurrentEndPoint.Name));
    ...
    public void Throws_when_resolving_unregistered_client()
    {
        ...
        Assert.That(ex is DependencyResolutionException, Is.True);
    }
    ...
    public void Uses_cached_endpoint_on_second_call()
    {
        ...
        Assert.That("Application_WebAPI1", Is.EqualTo(client2.CurrentEndPoint.Name));
        Assert.That("WebAPIClient1", Is.EqualTo(result2));
        Assert.That(2, Is.EqualTo(inProcessCalls));   // We should not test the in process endpoint again - we go directly to the cached HTTP endpoint.
        Assert.That(1, Is.EqualTo(webAPICalls));
    }
    ...
    public void Client_exception_is_propagated()
    {
        ...
        Assert.That("Application_SQL1", Is.EqualTo(client1.CurrentEndPoint.Name));
        Assert.That("InProcess Exception", Is.EqualTo(ex.Message));
    }
    ...
    public async Task CurrentEndPoint_is_thread_safe()
    {
        ...
        Assert.That("Application_SQL1", Is.EqualTo(client1.CurrentEndPoint.Name));
        Assert.That("InProcessClient1", Is.EqualTo(result1));
        int result2 = client2.Call(x => x.GetInt());
        Assert.That("Application_SQL2", Is.EqualTo(client2.CurrentEndPoint.Name));
        Assert.That(1, Is.EqualTo(result2));
        ...
        Task t1 = Task.Run(() => {
            ...
            for (int i = 0; i < 1000; i++)
            {
                ...
                Assert.That("Application_SQL1", Is.EqualTo(client1.CurrentEndPoint.Name));
                Assert.That("InProcessClient1", Is.EqualTo(r1));

                int r2 = client2.Call(x => x.GetInt());
                Assert.That("Application_SQL2", Is.EqualTo(client2.CurrentEndPoint.Name));
                Assert.That(1, Is.EqualTo(r2));
            }
        });
    ...
    public void EndPoint_is_validated_once_only_when_override_endpoint_is_passed()
    {
        ...
        Assert.That("Application_SQL1", Is.EqualTo(client1.CurrentEndPoint.Name));
        Assert.That("InProcessClient1", Is.EqualTo(result));
        Assert.That(1, Is.EqualTo(inProcessCalls));
        Assert.That(0, Is.EqualTo(webAPICalls));
        ...
        Assert.That("Application_WebAPI1", Is.EqualTo(client1.CurrentEndPoint.Name));
        Assert.That("WebAPIClient1", Is.EqualTo(result));
        Assert.That(1, Is.EqualTo(inProcessCalls));         // validate only once
        Assert.That(1, Is.EqualTo(webAPICalls));
